...
<!-- XSL Parameters   -->
<xsl:param name="timestamp1"/>
<xsl:param name="timestamp2"/>

<!-- Read in our configuration XML file -->
<xsl:variable name="configFile" select="document('../xml/CONFIG.xml')" />

<xsl:variable name="baseURL" select="$configFile/config/baseURL"/>


<xsl:variable name="atom-timestamp">
<xsl:value-of select="$timestamp1"/>T<xsl:value-of select="$timestamp2"/><xsl:value-of select="$configFile/config/UTC_timezone_offset"/> 
</xsl:variable>
